...
struct skynet_context {
	...
	int ref;
	char result[32];
	void * cb_ud;//xxx 服务内部的数据结构,如struct logger、struct master、struct gate
	skynet_cb cb;//xxx 服务的回调函数
	int session_id;
	uint32_t forward;
	struct message_queue *queue;//xxx 私有消息队列
	bool init;
	bool endless;
...
/**
@brief 根据服务的名称来启动一个服务
@param name	服务模块的名称logger
@param param 服务启动参数
@return 
*/
struct skynet_context * 
skynet_context_new(const char * name, const char *param) {
	...
	if (inst == NULL)
		return NULL;
	struct skynet_context * ctx = malloc(sizeof(*ctx));//xxx 每个服务对应一个ctx
	CHECKCALLING_INIT(ctx)

	ctx->mod = mod;//xxx 在ctx保存服务
	ctx->instance = inst;//xxx 
	ctx->ref = 2;//xxx 每个ctx初始引用计数为2
	ctx->cb = NULL;//服务的回调函数
	ctx->cb_ud = NULL;
	ctx->session_id = 0;
	...
	ctx->init = false;
	ctx->endless = false;
	ctx->handle = skynet_handle_register(ctx);//xxx 为服务分配一个handle用来标识该服务
	struct message_queue * queue = ctx->queue = skynet_mq_create(ctx->handle);//xxx 为服务生成私有消息队列
	...
	if (r == 0) {
		...
		skynet_mq_force_push(queue);//xxx 将模块的私有消息队列 加入全局消息队列
		if (ret) {
...
skynet_context_grab(struct skynet_context *ctx) {
	__sync_add_and_fetch(&ctx->ref,1);//xxx 请求一个服务后，其ctx的ref+1，这样能保证该服务不被skynet清理
}
...
skynet_context_release(struct skynet_context *ctx) {
	if (__sync_sub_and_fetch(&ctx->ref,1) == 0) {
		_delete_context(ctx);//xxx 如果一个服务ctx的ref=0 那么skynet将清理该服务
		return NULL;
	}
	return ctx;
}
...
_dispatch_message(struct skynet_context *ctx, struct skynet_message *msg) {
	assert(ctx->init);
	CHECKCALLING_BEGIN(ctx)
	int type = msg->sz >> HANDLE_REMOTE_SHIFT;//xxx 获取高八位数据，用于判断消息类型
	size_t sz = msg->sz & HANDLE_MASK;//xxx 低24位 用于标识消息字节数
	if (type == PTYPE_MULTICAST) {//xxx 各个skynet节点之间的广播消息
		skynet_multicast_dispatch((struct skynet_multicast_message *)msg->data, ctx, _mc);
	} else {
		int reserve = ctx->cb(ctx, ctx->cb_ud, type, msg->session, msg->source, msg->data, sz);//xxx 调用服务的回调函数来处理其消息
...
/**
@breif	一个待启动的服务向skynet注册
		一个消息处理回调函数
@param	context 服务对应的context
@param	ud	服务内部的结构指针 struct logger*
@param	cb	待注册的回调函数
@return void
*/
void 
...
/**
@breif	用于服务间消息的传递
@param	ctx	特定的服务对应的ctx
@param	msg	待发送的消息(发送方负责复制一次该数据，接收方清理)
@param	sz	待发送的msg的字节数
@param	source	发送方的handle id(每个服务唯一)
@param	type	
*/
void

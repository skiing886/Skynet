...
@param   xxx  The file name of *.plist file
...
reverve_id(struct socket_server *ss) {
	...
	for (i=0;i<MAX_SOCKET;i++) {
		int id = __sync_add_and_fetch(&(ss->alloc_id), 1);//xxx 线程安全的 ++(ss->alloc_id)
		if (id < 0) {
...
/**
@brief   初始化服务器，默认会使用epoll模型
		 使用pipe()函数为进程打开两个额外的文件描述符
		 fd[0]--读取  fd[1]--写入
		 并将fd[0]加入到epoll事件侦听中
@param   void  
@return  socket_server的一个指针
*/
struct socket_server * 
socket_server_create() {
	int i;
	int fd[2];
	poll_fd efd = sp_create();//xxx 使用epoll模型
	if (sp_invalid(efd)) {
